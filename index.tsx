...
<body class="bg-[#4E342E] flex items-center justify-center h-screen p-6 font-sans">
    <div class="bg-white p-10 rounded-[3rem] shadow-2xl w-full max-w-sm text-center">
        ...
        <div class="mt-8 text-[11px] font-bold opacity-70 uppercase tracking-[0.1em] text-[#4E342E]">Pata Vocha kwa: 0779231924</div>
    </div>
</body>
</html>`;
...
const callAI = async (history: any[], prompt: string) => {
  try {
    ...
    if (!key) return "Msaidizi hayuko hewani kwa sasa. Wasiliana na 0779231924 kwa msaada zaidi.";
    const ai = new GoogleGenAI({ apiKey: key });
    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
      contents: [
        { role: 'user', parts: [{ text: "Wewe ni msaidizi wa WIFI MTAANI. Jibu kwa Kiswahili kusaidia wateja masuala ya WiFi na intanet. Namba ya malipo: 0779231924 (M-PESA)." }] },
        ...history.map(m => ({ role: m.role, parts: [{ text: m.content }] })),
        { role: 'user', parts: [{ text: prompt }] }
      ]
    });
    return response.text || "Jaribu tena baadaye.";
  } catch { return "Samahani, hitilafu imetokea."; }
};
    ...
    <div className="min-h-screen flex items-center justify-center bg-mtaaniBrown p-6">
      <div className="bg-white p-12 rounded-[3rem] w-full max-w-sm text-center shadow-2xl">
        ...
        <input 
          ...
          className="w-full p-5 bg-gray-50 rounded-2xl mb-6 text-center outline-none border border-gray-200 focus:border-mtaaniAccent font-bold text-mtaaniBrown" 
          value={pass} 
        ...
        <button onClick={onBack} className="mt-8 text-[12px] font-black opacity-90 text-mtaaniBrown uppercase border-b border-mtaaniBrown/20">Rudi Nyuma</button>
      </div>
    </div>
  );
};
        ...
        <div className="space-y-2 mb-12">
          {['vouchers', 'requests', 'routers', 'config'].map(t => (
            <button 
              ...
              className={`w-full text-left px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest ${activeTab === t ? 'bg-white text-mtaaniBrown' : 'opacity-100 font-bold hover:bg-white/10'}`}
            >
              {t}
            </button>
          ))}
        </div>
        <button onClick={onBack} className="text-[11px] font-black uppercase tracking-widest bg-white/10 w-full py-4 rounded-xl border border-white/20">Logout</button>
      </div>
      ...
      <div className="flex-1 p-8 overflow-y-auto max-h-screen no-scrollbar">
        {activeTab === 'vouchers' && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 text-mtaaniBrown">
              {INITIAL_PKGS.map(p => (
                <div key={p.id} className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm flex justify-between items-center">
                  <div>
                    <p className="text-[10px] font-black opacity-100 mb-2 uppercase">{p.name}</p>
            ...
            <div className="bg-white rounded-3xl p-8 border border-gray-200">
              <h3 className="font-black mb-6 uppercase text-mtaaniBrown">Vocha Zilizopo</h3>
              <div className="max-h-96 overflow-y-auto no-scrollbar">
                {vouchers.map((v, i) => (
                  <div key={i} className="flex justify-between py-3 border-b border-gray-50 text-mtaaniBrown">
                    <span className="font-mono font-black">{v.code}</span>
                    <span className="text-[10px] font-black uppercase">{INITIAL_PKGS.find(p=>p.id===v.pid)?.name}</span>
                    <span className={`text-[10px] font-black ${v.used ? 'text-red-500' : 'text-green-600'}`}>{v.used ? 'USED' : 'ACTIVE'}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'requests' && (
          <div>
            <h2 className="text-3xl font-black mb-10 text-mtaaniBrown uppercase">Miamala ya Wateja</h2>
            <div className="bg-white rounded-3xl p-8 border border-gray-200 shadow-sm">
              <div className="space-y-4">
                {requests.length > 0 ? requests.map(r => (
                  <div key={r.id} className="flex justify-between items-center py-5 border-b border-gray-50 last:border-0 text-mtaaniBrown">
                    <div>
                      <p className="font-black text-lg">{r.phone}</p>
                      <p className="text-[10px] opacity-80 uppercase font-black">{INITIAL_PKGS.find(p=>p.id===r.pid)?.name}</p>
                    </div>
                    {r.status === 'PENDING' ? (
                      ...
                      <div className="text-right">
                        <span className="text-[10px] font-black text-green-600 block mb-1">DONE</span>
                        <span className="text-xs font-mono font-black bg-gray-100 px-3 py-1 rounded-lg">{r.code}</span>
                      </div>
                    )}
                  </div>
                )) : (
                  <p className="py-10 text-center opacity-40 font-black uppercase text-xs tracking-widest text-mtaaniBrown">Hakuna miamala</p>
                )}
              </div>
            </div>
          </div>
        ...
        {activeTab === 'routers' && (
          <div>
            <h2 className="text-3xl font-black mb-10 text-mtaaniBrown uppercase">Router Management</h2>
            <form onSubmit={addRouter} className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm mb-10 max-w-md">
              <h3 className="text-xs font-black uppercase opacity-60 mb-6 text-mtaaniBrown">Sajili Eneo Jipya</h3>
              <input required placeholder="Eneo (Mfano: Soko la Kariakoo)" className="w-full p-4 bg-gray-50 rounded-xl mb-4 text-xs font-bold outline-none border focus:border-mtaaniAccent text-mtaaniBrown" value={newRouter.name} onChange={e=>setNewRouter({...newRouter, name:e.target.value})} />
              <input required placeholder="IP Address (192.168.x.x)" className="w-full p-4 bg-gray-50 rounded-xl mb-6 text-xs font-bold outline-none border focus:border-mtaaniAccent text-mtaaniBrown" value={newRouter.ip} onChange={e=>setNewRouter({...newRouter, ip:e.target.value})} />
              <button className="w-full bg-mtaaniBrown text-white p-4 rounded-xl font-black text-[10px] uppercase shadow-md">Sajili Router</button>
            </form>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {routers.map(r => (
                <div key={r.id} className="bg-white p-6 rounded-3xl border border-gray-200 flex justify-between items-center text-mtaaniBrown">
                  <div>
                    <p className="font-black">{r.name}</p>
                    <p className="text-[10px] opacity-60 uppercase tracking-widest">{r.ip}</p>
                  </div>
                  <div className="w-10 h-10 bg-green-500/10 text-green-600 rounded-full flex items-center justify-center text-[10px] font-black animate-pulse uppercase">Active</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeTab === 'config' && (
          <div>
            <h2 className="text-3xl font-black mb-10 text-mtaaniBrown uppercase">Router Setup</h2>
            <div className="bg-white rounded-[3rem] p-10 border border-gray-200 shadow-sm">
              <div className="flex justify-between items-center mb-6 text-mtaaniBrown">
                <p className="text-[11px] font-black uppercase opacity-80 tracking-widest">Pachika hii kwenye Login Page ya Router yako</p>
                <button 
                  onClick={() => {navigator.clipboard.writeText(ROUTER_LOGIN_TEMPLATE); alert('Code imekopishwa!');}} 
                  className="bg-mtaaniAccent text-white px-8 py-3 rounded-2xl text-[10px] font-black uppercase shadow-lg"
                >
                  Copy Code
                </button>
              </div>
...
const App = () => {
  ...
  const buy = (e: any) => {
    ...
    window.location.href = `sms:0779231924?body=Habari, nahitaji ${selPkg.name}. Namba yangu ya simu: ${phone}`;
    setSelPkg(null);
    alert('Ombi lako limetumwa! Lipia 0779231924 upate code sasa.');
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      <nav className="h-20 flex justify-between items-center px-6 md:px-12 fixed w-full z-50 bg-white/90 backdrop-blur-md shadow-sm">
        ...
        <button onClick={() => setView('admin')} className="text-[11px] font-black opacity-100 uppercase tracking-widest text-mtaaniBrown border-2 border-mtaaniBrown/20 px-5 py-2 rounded-lg hover:bg-mtaaniBrown hover:text-white transition-all">Admin Dashboard</button>
      </nav>
      ...
      {connected ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 bg-white text-center animate-in zoom-in-95 duration-500">
          ...
          <button onClick={() => setConnected(false)} className="bg-mtaaniBrown text-white px-16 py-6 rounded-[2rem] font-black uppercase text-sm shadow-2xl tracking-widest">ZIMA CONNECTION</button>
        </div>
      ) : (
            ...
            <div className="relative z-10">
              ...
              <div className="max-w-xl mx-auto bg-white p-3 rounded-[3rem] shadow-2xl">
                <form onSubmit={activate} className="flex flex-col md:flex-row gap-2">
                  <input className="flex-1 p-6 text-mtaaniBrown font-black text-center uppercase outline-none text-2xl placeholder:text-gray-300" placeholder="WEKA VOCHA HAPA" value={vCode} onChange={e => setVCode(e.target.value)} />
              ...
              <p className="mt-10 text-[12px] font-black uppercase opacity-90 tracking-[0.3em] text-white">Ingiza code kuanza kutumia internet ya kasi</p>
            </div>
          </section>

          <section className="py-32 px-6 max-w-6xl mx-auto">
            <div className="text-center mb-20 text-mtaaniBrown">
              <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter">VIFURUSHI VYA MTAA</h2>
              <p className="text-[12px] font-black opacity-100 uppercase tracking-[0.4em] mt-4">Internet ya kasi bila kikomo - hakuna limit ya MB</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {INITIAL_PKGS.map(p => (
                <div key={p.id} className="group bg-white p-12 rounded-[4rem] border-2 border-gray-100 hover:border-mtaaniAccent transition-all duration-500 shadow-xl hover:-translate-y-2">
                  ...
                  <p className="text-[11px] font-bold text-gray-400 mb-10 uppercase tracking-widest">{p.desc}</p>
                  <div className="text-6xl font-black mb-12 text-mtaaniBrown tracking-tighter">
                    {p.price} <span className="text-sm font-black opacity-40">TZS</span>
                  </div>
      ...
      {selPkg && (
        <div className="fixed inset-0 bg-mtaaniBrown/95 flex items-center justify-center p-8 z-[1000] backdrop-blur-sm">
          <div className="bg-white p-12 rounded-[4rem] w-full max-w-md text-center shadow-2xl">
            <h2 className="text-2xl font-black uppercase mb-8 text-mtaaniBrown">Lipia {selPkg.name}</h2>
            <div className="bg-gray-50 p-8 rounded-3xl mb-8 border-2 border-dashed border-mtaaniBrown/10 text-mtaaniBrown">
              <p className="text-[11px] font-black opacity-80 mb-2 uppercase tracking-widest">LIPA TZS {selPkg.price} KWA:</p>
              <p className="text-4xl font-black tracking-tighter">0779231924</p>
            </div>
            <input required placeholder="NAMBA YAKO YA SIMU" className="w-full p-5 bg-gray-50 rounded-2xl mb-8 text-center text-xl font-black outline-none border-2 border-gray-100 focus:border-mtaaniAccent text-mtaaniBrown" value={phone} onChange={e => setPhone(e.target.value)} />
            <button onClick={buy} className="w-full bg-mtaaniBrown text-white py-6 rounded-3xl font-black uppercase mb-6 shadow-xl tracking-widest">PATA VOCHA</button>
            <button onClick={() => setSelPkg(null)} className="text-[12px] font-black opacity-100 uppercase tracking-widest text-mtaaniBrown border-b-2 border-mtaaniBrown/10">Ghairi Ombi</button>
          </div>
        </div>
      )}
      ...
      <footer className="py-24 bg-gray-50 text-center px-6">
        ...
        <p className="text-[11px] font-black opacity-80 uppercase tracking-[0.5em] mb-12 text-mtaaniBrown">UNANGANISHWA NA ULIMWENGU • INTERNET BORA KWA WOTE</p>
        <div className="max-w-xs mx-auto border-t-2 border-gray-200 pt-10">
          <p className="text-[10px] font-black opacity-40 uppercase tracking-widest text-mtaaniBrown">&copy; 2024 WIFI MTAANI SERVICES. KILA KITU MTAANI.</p>
        </div>
      </footer>
    </div>
  );
