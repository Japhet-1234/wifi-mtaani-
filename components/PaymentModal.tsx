...
const PaymentModal: React.FC<PaymentModalProps> = ({ pkg, onClose }) => {
  ...
  const handleRequestVoucher = (e: React.FormEvent) => {
    ...
    const message = `Habari WIFI MTAANI. Nahitaji vocha ya ${pkg.name} (${pkg.price} TZS). Namba yangu ya malipo ni ${phone}.`;
        ...
        <div className="p-8">
          {!requested ? (
            <div className="space-y-6">
              {/* MANDATORY INSTRUCTION - HIGHLIGHTED */}
              <div className="bg-orange-50 border-2 border-orange-200 p-6 rounded-3xl text-center">
                <p className="text-orange-800 font-black text-lg leading-tight">
                  TAFADHALI LIPIA KWA SIMU KWANZA KWENYE NAMBA <span className="text-orange-600 block text-2xl mt-1">0779231924</span>
                </p>
              </div>
              ...
              <div className="bg-mtaaniGrey/20 p-6 rounded-3xl space-y-4">
                <div className="flex gap-4">
                  ...
                  <p className="text-sm font-bold text-gray-700">Tuma <b>TZS {pkg.price.toLocaleString()}</b> kwenda namba hiyo hapo juu.</p>
                </div>
                
                <div className="bg-white p-3 rounded-2xl text-center">
                   <p className="text-[10px] font-black text-mtaaniBrown/40 uppercase tracking-widest">Jina la Malipo:</p>
                   <p className="font-black text-mtaaniBrown uppercase">WIFI MTAANI SERVICES</p>
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <p className="text-sm font-bold text-gray-700">Kisha weka namba yako uliyotumia kulipia:</p>
                </div>
              </div>
              ...
              <form onSubmit={handleRequestVoucher} className="space-y-4">
                ...
                <button 
                  type="submit"
                  className="w-full bg-mtaaniBrown text-mtaaniGrey py-5 rounded-2xl font-black text-xl hover:scale-[1.02] active:scale-95 transition-all shadow-xl"
                >
                  TUMA OMBI LA VOCHA
                </button>
                <p className="text-[10px] text-center font-bold uppercase opacity-40">
                  UTATUMIWA VOCHA CODE KWA SMS MARA MOJA
                </p>
              </form>
            </div>
          ) : (
            <div className="text-center py-6 animate-in fade-in duration-500">
              ...
              <h4 className="text-3xl font-black text-mtaaniBrown mb-4">Ombi Limetumwa!</h4>
              <p className="text-gray-500 font-medium mb-10 leading-relaxed">Admin amepokea ombi lako. Hakikisha umetuma SMS uliyopelekewa ili upate vocha yako sasa hivi.</p>
              
              <button 
                onClick={onClose}
                className="w-full bg-mtaaniBrown text-mtaaniGrey py-4 rounded-2xl font-black text-lg shadow-lg"
              >
                SAWA, NIMEFAHAMU
              </button>
            </div>
